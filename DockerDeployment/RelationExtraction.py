...
from flairrelex.data import Sentence, Token
...
class RelationExtractionModel(object):
    def __init__(self):
        ...
        #self.clf = TextClassifier.load_from_file('flairrelex.pt')
        ...
        self.words = ['Wegen', 'Hypocalciaemie', 'stationaer', 'auf', 'der', '134', 'vom', '22.7', '.', 'bis', '3.8.04',
                      '.', 'Gutes', 'Befinden', ',', 'keine', 'Kraempfe', ',', 'keine', 'GI-Symptome', ',', 'kein',
                      'epigastrischen', 'Schmerzen', ',', 'Stuhlgang', 'normal', '.']

        self.tags = ['O', 'B-Medical_condition', 'B-Treatment', 'O', 'O', 'O', 'B-Time_information',
                     'I-Time_information', 'I-Time_information', 'I-Time_information', 'I-Time_information', 'O',
                     'B-State_of_health', 'I-State_of_health', 'O', 'O', 'B-Medical_condition', 'O', 'O',
                     'B-Medical_condition', 'O', 'O', 'B-Local_specification', 'B-Medical_condition', 'O', 'B-Process',
                     'B-State_of_health', 'O']
        # self.doctemp = Doc(self.nlp.vocab, words=words, spaces=spaces, sent_starts=heads)

    def preprocess_input(self, doc, tags_per_sent):
        pass

    def create_doc(self, token_list, sent_starts_index):
        #print(type(int(sent_starts_index[0])))
        #sent_starts_index = list(map(lambda k: int(k), sent_starts_index))
    ...
    def _prepare_sentence(sent, entity1, entity2, map_dict):
        ...
        add_concept_to_sentence(sentence, (entity1.start - sent_offset, entity1.end - sent_offset), tag='concept_1',
                                tag_value=map_dict[entity1.label_])
        add_concept_to_sentence(sentence, (entity2.start - sent_offset, entity2.end - sent_offset), tag='concept_2',
                                tag_value=map_dict[entity2.label_])
    ...
    def predict(self, token_list, sent_starts_index, tags):
        ...
        sentences = []
        entity_combinations = []
        tt = list(zip(doc.sents, entity_groups))
        ...
        for sent, (ent1, ent2) in zip(pred_sentences, entity_combinations):
            ...
            if not negative_relation(relation):
                if swap_entities(relation):
                    ...
                    rel_res.append(''.join([t.text for t in ent2])+' '+ent2.label_+' '+''.join([t.text for t in ent1])+' '+ent1.label_+' '+relation_name(relation))
                else:
                    relations.append((ent1, ent2, relation_name(relation)))
                    rel_res.append(''.join([t.text for t in ent1])+' '+ent1.label_+' '+''.join([t.text for t in ent2])+' '+ent2.label_+' '+relation_name(relation))
...
if __name__ == '__main__':
    ...
    #a = TextClassifier.load_from_file('relex.pt')
    #b = a['label_dictionary']

    obj = RelationExtractionModel()
    a, b = obj.predict(obj.words, [11, 17], obj.tags)
